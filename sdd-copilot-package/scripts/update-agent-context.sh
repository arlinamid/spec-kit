...
NEW_SECURITY_LEVEL=$(grep "^**Security Level**: " "$NEW_PLAN" 2>/dev/null | head -1 | sed 's/^**Security Level**: //' | grep -v "NEEDS CLARIFICATION" || echo "")
NEW_SECURITY_DEPS=$(grep "^**Security Dependencies**: " "$NEW_PLAN" 2>/dev/null | head -1 | sed 's/^**Security Dependencies**: //' | grep -v "NEEDS CLARIFICATION" || echo "")
NEW_SECURITY_TESTING=$(grep "^**Security Testing**: " "$NEW_PLAN" 2>/dev/null | head -1 | sed 's/^**Security Testing**: //' | grep -v "NEEDS CLARIFICATION" || echo "")
NEW_SECURITY_GOALS=$(grep "^**Security Goals**: " "$NEW_PLAN" 2>/dev/null | head -1 | sed 's/^**Security Goals**: //' | grep -v "NEEDS CLARIFICATION" || echo "")
NEW_SECURITY_CONSTRAINTS=$(grep "^**Security Constraints**: " "$NEW_PLAN" 2>/dev/null | head -1 | sed 's/^**Security Constraints**: //' | grep -v "NEEDS CLARIFICATION" || echo "")
