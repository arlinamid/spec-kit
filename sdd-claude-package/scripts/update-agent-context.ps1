...
# Extract security context from new plan
$newSecurityLevel = ""
$newSecurityDeps = ""
$newSecurityTesting = ""
$newSecurityGoals = ""
...
if (Test-Path $newPlan -PathType Leaf) {
    ...
    if ($planContent -match '\*\*Security Level\*\*: (.+)') {
        $newSecurityLevel = $matches[1].Trim()
        if ($newSecurityLevel -eq "NEEDS CLARIFICATION") { $newSecurityLevel = "" }
    }
    
    # Extract security dependencies
    if ($planContent -match '\*\*Security Dependencies\*\*: (.+)') {
        $newSecurityDeps = $matches[1].Trim()
        if ($newSecurityDeps -eq "NEEDS CLARIFICATION") { $newSecurityDeps = "" }
    }
    
    # Extract security testing
    if ($planContent -match '\*\*Security Testing\*\*: (.+)') {
        $newSecurityTesting = $matches[1].Trim()
        if ($newSecurityTesting -eq "NEEDS CLARIFICATION") { $newSecurityTesting = "" }
    }
    
    # Extract security goals
    if ($planContent -match '\*\*Security Goals\*\*: (.+)') {
        $newSecurityGoals = $matches[1].Trim()
        if ($newSecurityGoals -eq "NEEDS CLARIFICATION") { $newSecurityGoals = "" }
    }
    
    # Extract security constraints
    if ($planContent -match '\*\*Security Constraints\*\*: (.+)') {
        $newSecurityConstraints = $matches[1].Trim()
        if ($newSecurityConstraints -eq "NEEDS CLARIFICATION") { $newSecurityConstraints = "" }
    }
}
